...
const { Suppliers } = cds.entities('sap.ariba');
    ...
    return new Promise(async function(resolve, reject)    {
        ...
        for(const oData of aData) {
            ...
            try {
                ...
                let res =  await srv.run ( SELECT.from (Suppliers).where(
                    {
                 ...
                 if(res.length==0){
                     //New record, insert
                    await srv.run( INSERT .into (Suppliers) .entries (oDataCleansed) );

                 }else{
                     //Update existing record
                    await srv.run ( UPDATE (Suppliers) .set (oDataCleansed) .where(
                        {
                            Realm : oDataCleansed.Realm ,
                            SupplierId : oDataCleansed.SupplierId} )
                     );
